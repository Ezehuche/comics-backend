...
import { actionman } from './serFonts';
...
export async function injectSpeechBubbleInTheBackground(params: {
  ...
  const modelUrl =
    'https://www.kaggle.com/models/tensorflow/deeplab/tfjs/pascal/1/model.json?tfjs-format=file';
  ...
  const resized = tf.image.resizeBilinear(imgTensor, [513, 513]);
  const int32Tensor = resized.toInt().expandDims(0);

  console.log('Final Input Shape:', int32Tensor.shape);
  console.log('Final Input DType:', int32Tensor.dtype);
  console.log('Model Input Details:', model.inputs);

  // Run inference
  const predictions = model.execute({ ImageTensor: int32Tensor }) as tf.Tensor;
  console.log('Output Shape:', predictions.shape);

  // Convert output to segmentation mask
  const maskTensor = predictions.squeeze().toInt(); // Convert to int32 mask
  console.log('Final Mask Shape:', maskTensor.shape);

  const mask = maskTensor.arraySync() as number[][];
  let characterBoundingBox: BoundingBox | null = null;
  const maskArray = new Uint8Array(mask.flat());
  characterBoundingBox = findCharacterBoundingBox(
    maskArray,
    image.width,
    image.height,
  );
  ...
  if (debug) {
    drawSegmentationMask(ctx, maskArray, image.width, image.height);
  }

  const bubbles = splitTextIntoBubbles(text);
  const bubbleLocations = calculateBubbleLocations(
    bubbles.length,
    image.width,
    image.height,
...
// function loadImageFromBase64(base64: string): Promise<Image> {
...
function findCharacterBoundingBox(
  ...
  for (let y = 0; y < height; y++) {
    for (let x = 0; x < width; x++) {
      ...
      if (mask[index] > 0 && !visited.has(index)) {
        const shape = floodFill(mask, width, height, x, y, visited);
        shapes.push(shape);
      }
    }
  }
  ...
  const mostVerticalShape = shapes.reduce((prev, current) => {
  ...
  for (let i = 0; i < bubbleCount; i++) {
    ...
    do {
      ...
      if (characterBoundingBox) {
        const characterMiddle =
          characterBoundingBox.left + characterBoundingBox.width / 2;
        const leftSide = Math.random() * (characterMiddle - padding - padding);
        const rightSide =
          characterMiddle +
          Math.random() * (imageWidth - characterMiddle - padding - padding);
      ...
      if (attempts >= maxAttempts) {
        console.warn(
          `Could not find non-overlapping position for bubble ${i} after ${maxAttempts} attempts.`,
        );
        break;
      }
    } while (
      characterBoundingBox &&
      isOverlapping({ x, y }, characterBoundingBox)
    );
...
function drawSpeechBubble(
  ...
  const borderPadding = Math.max(
    10,
    Math.min(imageWidth, imageHeight) * safetyMargin,
  );
  ...
  const finalWidth = Math.min(
    Math.max(textDimensions.width + padding * 2, 100),
    maxBubbleWidth,
  );
  const finalHeight = Math.min(
    Math.max(textDimensions.height + padding * 2, 50),
    imageHeight - 2 * borderPadding,
  );

  const bubbleLocation = adjustBubbleLocation(
    location,
    finalWidth,
    finalHeight,
    characterBoundingBox,
...
function adjustBubbleLocation(
  ...
  if (characterBoundingBox) {
    ...
    if (Math.abs(adjustedX - characterMiddle) < width / 2) {
      ...
      adjustedX =
        adjustedX < characterMiddle
          ? Math.max(
              width / 2 + borderPadding,
              characterBoundingBox.left - width / 2 - 10,
            )
          : Math.min(
              imageWidth - width / 2 - borderPadding,
              characterBoundingBox.left +
  ...
  adjustedX = Math.max(
    width / 2 + borderPadding,
    Math.min(imageWidth - width / 2 - borderPadding, adjustedX),
  );
  adjustedY = Math.max(
    height / 2 + borderPadding,
    Math.min(imageHeight - height / 2 - borderPadding, adjustedY),
  );
...
function drawRectangularBubble(
  ctx: CanvasRenderingContext2D,
  location: { x: number; y: number },
  width: number,
  height: number,
