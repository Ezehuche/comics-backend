...
export const defaultLLMEngineName =
  `${process.env.LLM_ENGINE || ''}` as LLMEngine;

export function getLLMEngineFunction(
  llmEngineName: LLMEngine = defaultLLMEngineName,
) {
  const llmEngineFunction =
    llmEngineName === 'GROQ'
      ? predictWithGroq
      : llmEngineName === 'ANTHROPIC'
        ? predictWithAnthropic
