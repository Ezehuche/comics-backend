...
const serverRenderingEngine =
  `${process.env.RENDERING_ENGINE || ''}` as RenderingEngine;
...
export async function newRender({
  ...
  const defaulResult: RenderedScene = {
    renderId: '',
    status: 'error',
    assetUrl: '',
    alt: prompt || '',
  ...
  const openaiApiKey = serverOpenaiApiKey;
  ...
  const huggingfaceApiUrl = serverHuggingfaceApiUrl;
  const huggingfaceInferenceApiModelRefinerModel =
  ...
  try {
    if (renderingEngine === 'REPLICATE') {
      ...
      if (output) {
        return {
          renderId,
          status: generatedResp?.error
            ? 'error'
            : generatedResp?.status === 'succeeded'
              ? 'completed'
              : 'pending',
          assetUrl: `${generatedResp?.output || ''}`,
          alt: `${generatedResp?.input?.prompt || ''}`,
          error: `${generatedResp?.error || ''}`,
          maskUrl: '',
          segments: [],
        } as RenderedScene;
      }
